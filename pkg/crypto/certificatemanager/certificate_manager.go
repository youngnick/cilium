...
func (m *manager) GetSecrets(ctx context.Context, secret *api.Secret, ns string) (string, map[string][]byte, bool, error) {
	...
	// If secret syncing is enabled, then substitute the synchronized secret location for the original one.
	if m.secretSyncNamespace != "" {
		secrets, err := m.k8sClient.GetSecrets(ctx, m.secretSyncNamespace, fmt.Sprintf("%s-%s", ns, secret.Name))
		// TODO: Fix this. There's a race between the operator creating the synchronized secret and the agent reading it.
		if err != nil {
			time.Sleep(time.Millisecond * 200)
			secrets, err = m.k8sClient.GetSecrets(ctx, m.secretSyncNamespace, fmt.Sprintf("%s-%s", ns, secret.Name))
		}
		return nsName, secrets, false, err
	}

	// values read from k8s, so bool returnval is false
