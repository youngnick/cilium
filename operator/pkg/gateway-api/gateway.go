...
func (r *gatewayReconciler) SetupWithManager(mgr ctrl.Manager) error {
	for indexName, indexerFunc := range map[string]client.IndexerFunc{
		backendServiceHTTPRouteIndex:       r.indexHTTPRouteByBackendService,
		backendServiceImportHTTPRouteIndex: r.indexHTTPRouteByBackendServiceImport,
		gatewayHTTPRouteIndex:              r.indexHTTPRouteByGateway,
	} {
	...
	if err := mgr.GetFieldIndexer().IndexField(context.Background(), &gatewayv1.Gateway{}, implementationGatewayIndex, r.indexGatewayByImplementation); err != nil {
...
func (r *gatewayReconciler) routeHasMatchingGatewayParent(parentRefs []gatewayv1.ParentReference, routeNamespace string) bool {
	for _, parent := range parentRefs {
		if r.parentIsMatchingGateway(parent, routeNamespace) {
			return true
		}
	}
	return false
...
// indexHTTPRouteByBackendService takes a single HTTPRoute and  returns all referenced backend service full names (`namespace/name`)
// to add to the relevant index.
func (r *gatewayReconciler) indexHTTPRouteByBackendService(rawObj client.Object) []string {
...
// indexHTTPRouteByBackendServiceImport takes a single HTTPRoute and  returns all referenced backend ServiceImport full names (`namespace/name`)
// to add to the relevant index.
func (r *gatewayReconciler) indexHTTPRouteByBackendServiceImport(rawObj client.Object) []string {
	...
	if r.routeHasMatchingGatewayParent(hr.Spec.ParentRefs, hr.Namespace) {
...
// indexHTTPRouteByGateway takes a single HTTPRoute and returns all referenced Gateway object full names (`namespace/name`)
// to add to the relevant index.
//
// Note that this does _not_ filter to only Cilium-relevant Gateways.
func (r *gatewayReconciler) indexHTTPRouteByGateway(rawObj client.Object) []string {
...
func (r *gatewayReconciler) indexGatewayByImplementation(rawObj client.Object) []string {
	gw := rawObj.(*gatewayv1.Gateway)

	gwc := &gatewayv1.GatewayClass{}
	if err := r.Client.Get(context.Background(), types.NamespacedName{Name: string(gw.Spec.GatewayClassName)}, gwc); err != nil {
...
// enqueueRequestForOwningResource returns an event handler that, when given any object with an
// owningGatewayLabel, returns reconcile.Requests for that Gateway.
...
func (r *gatewayReconciler) enqueueRequestForBackendService() handler.EventHandler {
	return handler.EnqueueRequestsFromMapFunc(func(ctx context.Context, o client.Object) []reconcile.Request {
		...
		scopedLog := r.logger.With("controller-subsys", "queue-gw-from-svc")
		...
		if err := r.Client.List(ctx, gwList, &client.ListOptions{
			...
			scopedLog.Error("Failed to get Cilium Gateways", logfields.Error, err)
			return []reconcile.Request{}
		}
