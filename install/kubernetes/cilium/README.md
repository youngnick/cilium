...
| tls.secretSync.enabled | bool | `true` | Enable synchronziation of Secrets for TLS Interception. If disabled and tls.secretsBackend is set to 'k8s', then secrets will be read directly by the agent. |
...
| tls.secretSync.secretsNamespace.name | string | `"cilium-secrets"` | Name of Gateway API secret namespace. |
